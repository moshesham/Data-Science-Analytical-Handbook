...
fig, axes = plt.subplots(
    3, 2, figsize=(15, 12)
)  # 3 rows (one for each distribution), 2 columns (hist and box)
...
x = np.linspace(
    stats.norm.ppf(0.001, loc=mu, scale=sigma),
...
plt.axvline(
    ...
    linestyle="dashed",
    linewidth=1,
    label=f"Median: {np.median(normal_data):.2f}",
)
...
plt.text(
    0.5,
    -0.2,
    "Histogram of generated data from a normal distribution. The mean and median are also shown as dashed lines.",
    ha="center",
    va="top",
...
plt.vlines(
    x_poisson, 0, stats.poisson.pmf(x_poisson, mu=lam), colors="b", lw=2, alpha=0.5
)
...
plt.axvline(
    np.median(poisson_data),
    color="g",
    linestyle="dashed",
    linewidth=1,
    label=f"Median: {np.median(poisson_data):.2f}",
)
...
plt.text(
    0.5,
    -0.2,
    "Histogram of generated Poisson data. The mean and median are shown as dashed lines.",
    ha="center",
    va="top",
    transform=plt.gca().transAxes,
)
...
x_expon = np.linspace(
    stats.expon.ppf(0.001, scale=scale), stats.expon.ppf(0.999, scale=scale), 100
)
...
plt.figure(figsize=(8, 6))
plt.axvline(
    scale, color="b", linestyle="dashed", linewidth=1, label=f"Mean: {scale:.2f}"
)
plt.axvline(
    np.median(exponential_data),
    color="g",
    linestyle="dashed",
    linewidth=1,
    label=f"Median: {np.median(exponential_data):.2f}",
)
...
plt.text(
    0.5,
    -0.2,
    "Histogram of generated exponential data. The mean and median are represented by dashed lines.",
    ha="center",
    va="top",
    transform=plt.gca().transAxes,
)
...
try:
    ...
    print("\nDataset Description:")
    print(
        "This dataset contains booking information for a city hotel and a resort hotel. We will be analyzing the relationships between some of the numerical features related to booking demand."
    )
    ...
    print("\nStays and Nights:")
    print(
        "- There is a strong positive correlation between `stays_in_weekend_nights` and `stays_in_week_nights`, as expected. Longer stays naturally involve more nights of both types."
    )

    print("\nAdults, Children, and Babies:")
    print(
        "- There are positive correlations between `adults`, `children`, and `babies`, indicating that bookings with more adults tend to also have more children and babies."
    )

    print("\nLead Time and Previous Cancellations:")
    print(
        "- There is a positive correlation between `lead_time` and `previous_cancellations`. This suggests that bookings made further in advance are more likely to be canceled."
    )

    print("\nADR and other features:")
    print(
        "- There is a weak positive correlation between `adr` (Average Daily Rate) and `adults`, indicating that bookings with more adults tend to have slightly higher room rates."
    )

    print("\nImportant Note:")
    print(
        "Correlation does not imply causation. These relationships are suggestive, not definitive. Further analysis and domain knowledge are needed to establish causality."
    )

except FileNotFoundError:
    print(
        f"Error: File not found at {data_path}. Please ensure the file exists in the 'data' folder in the same directory as the notebook."
    )
except pd.errors.ParserError:
    print(
        f"Error: Could not parse the CSV file at {data_path}. Please check the file format."
    )
...
true_intercept = 5
Y = (
    true_slope * X + true_intercept + np.random.normal(0, 2, n_samples)
...
plt.figure(figsize=(8, 6))
plt.scatter(
    X[:, 1], Y, label="Data Points"
)  # X is array with constant, plot the second column
...
def simulate_metric(group, network, treatment=False):
    ...
    for user in group:
        ...
        if treatment:
            ...
            for neighbor in network.loc[user][
                network.loc[user] == 1
            ].index:  # Network/Spillover effect
...
control_clusters = [
    cluster for cluster in cluster_ids if cluster not in treatment_clusters
]

# assign users to treatment and control
treatment_group = [
    user
    for user, cluster in user_cluster_assignments.items()
    if cluster in treatment_clusters
]
control_group = [
    user
    for user, cluster in user_cluster_assignments.items()
    if cluster in control_clusters
]
...
print(
    f"Average treatment metrics: {np.mean(list(treatment_metrics_cluster.values())):.2f}"
)
...
print(
    f"95% Confidence Interval: {confidence_interval[0]:.3f}, {confidence_interval[1]:.3f}"
)
...
plt.axvline(
    mean_estimate, color="red", linestyle="dashed", label=f"Mean: {mean_estimate:.2f}"
)
...
print(
    f"P(A|B) Probability of a user clicking an ad given they purchase: {P_A_given_B:.2f}"
)
...
print(
    f"Normal Distribution Simulated Data Mean: {np.mean(normal_data):.2f}, Std. Dev: {np.std(normal_data):.2f}"
)
...
for i, sample_means_for_size in enumerate(sample_means):
    ...
    sns.histplot(
        sample_means_for_size, kde=True, label=f"Sample Size: {sample_sizes[i]}"
    )
...
if p_value < alpha:
    print(
        "Reject null hypothesis. The mean is significantly different from the population mean"
    )
else:
...
sample_size = 30
sample_product_a = np.random.normal(
    loc=11, scale=3, size=sample_size
)  # Simulate data from product A
...
if p_value < alpha:
    print(
        "Reject the null hypothesis. The mean from Product A is different from the population mean."
    )
else:
    print(
...
population_std = 3
z_stat = (np.mean(sample_product_a) - population_mean) / (
    population_std / np.sqrt(sample_size)
)  # calculate z-stat
p_value_z = 2 * (
    1 - stats.norm.cdf(np.abs(z_stat))
...
if p_value < alpha:
    ...
    print(
        "Fail to reject the null hypothesis. There is not a statistically significant difference between group A and group B."
    )
...
if p_value_paired < alpha:
    ...
    print(
        "Fail to reject null. There is not a significant difference between before and after mean."
    )
...
engagement = 0.6 * hours_spent + np.random.normal(
    0, 2, num_users
)  # Engagement is correlated with hours, but not a direct effect
...
treatment_group = np.random.normal(
    11, 2, 100
)  # User engagement scores for group B (with treatment)
...
effect_size = (
    0.3  # Estimated effect size, must be given by us (difference between two means)
)
...
print(
    f"Sample Size required for each group (for alpha = {alpha}, power = {power}, effect_size = {effect_size}): {sample_size}"
)
...
for eff_size in effect_sizes:
    sample_size_eff = analysis.solve_power(
...
if p_value < alpha:
    ...
    print(
        "Fail to reject null hypothesis. There is not a significant difference between sample A and sample B"
    )
...
confidence_interval = stats.t.interval(
    confidence_level, len(data) - 1, loc=sample_mean, scale=sample_std_error
)

print(f"Sample Mean: {sample_mean:.3f}")
print(
    f"Confidence Interval ({confidence_level * 100}%) : {confidence_interval[0]:.3f}, {confidence_interval[1]:.3f}"
)
...
true_intercept = 5
Y = (
    true_slope * X + true_intercept + np.random.normal(0, 2, 100)
)  # Dependent variable with noise
...
true_intercept = 3
y_multiple = (
    np.dot(X, true_coef) + true_intercept + np.random.normal(0, 2, 100)
)  # Dependent
