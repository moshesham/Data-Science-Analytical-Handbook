...
def generate_time_series_data(start_date: str = "2023-01-01", periods: int = 365, freq: str = 'D',
                             base_value: float = 100, trend: float = 0.1, seasonality_amplitude: float = 10,
                              noise_scale: float = 5 ) -> pd.DataFrame:
        ...
        seasonal_component = seasonality_amplitude * np.sin(2 * np.pi * time_index / 30) #Monthly seasonality
        noise_component = np.random.normal(0, noise_scale, periods)

        time_series_values = base_value + trend_component + seasonal_component + noise_component
