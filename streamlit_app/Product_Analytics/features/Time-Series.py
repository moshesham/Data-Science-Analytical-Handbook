...
def generate_time_series_data(
    ...
    date_rng = pd.date_range(
        start="2023-01-01", periods=n_periods, freq="M"
    )  # monthly data
...
def main():
    ...
    if data_source == "Generate Synthetic Data":
        n_periods = st.slider(
            "Number of Periods:", min_value=50, max_value=500, value=200, step=50
        )
        trend_slope = st.slider(
            "Trend Slope:", min_value=-1.0, max_value=1.0, value=0.1, step=0.05
        )
        seasonality_strength = st.slider(
            "Seasonality Strength:", min_value=0, max_value=50, value=20, step=5
        )
        noise_level = st.slider(
            "Noise Level:", min_value=0, max_value=50, value=10, step=5
        )
        df = generate_time_series_data(
            n_periods, trend_slope, seasonality_strength, noise_level
        )
        ...
        if uploaded_file is not None:
            try:
                ...
                if df.index.isnull().any():
                    st.error(
                        "Date parsing failed. Please ensure your CSV has a valid date column as the first column."
                    )
                    df = None
    ...
    if df is not None:  # only proceed if df is valid
        ...
        if model_choice == "Manual ARIMA":
            ...
            try:
                ...
                ax.plot(
                    test.index, predictions, label="Predictions"
                )  # Use correct index
                ax.set_xlabel("Date")
                ax.set_ylabel("Value")
                ax.legend()
    ...
    quiz_questions = [
        ...
        },
        {
            ...
            "options": [
                ...
                "The time series has a strong trend",
                "The time series has strong seasonality",
            ],
        ...
        },
        {
            ...
            "options": [
                ...
                "Autocorrelated Regression Integrated Moving Average",
                "Autoregressive Integrated Moving Assessment",
                "Autocorrelated Regression Integrated Moving Assessment",
            ],
        ...
        },
        {
            ...
            "options": [
                ...
                "The order of the autoregressive (AR) component",
        ...
        },
        {
            ...
            "options": [
                "Autocorrelation measures the correlation with past values, while partial autocorrelation measures the correlation with future values.",
        ...
        },
        {
            ...
            "options": [
                ...
                "Repeating patterns within a fixed period",
                "The randomness or noise in the data after removing trend and seasonality",
                "Fluctuations that are not of a fixed frequency",
            ],
