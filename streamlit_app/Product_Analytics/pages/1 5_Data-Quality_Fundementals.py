...
def main():
    st.set_page_config(
        page_title="Data Governance Essentials", page_icon="🛡️", layout="wide"
    )
    ...
    topic = st.selectbox(
        "Select a Topic:",
        ["Data Quality", "Data Legacy Management", "Compliance Frameworks"],
    )
    ...
    if topic == "Data Quality":
        ...
        if dimension == "Accuracy":
            ...
            with st.expander("Interactive Example"):
                ...
                entered_address = st.text_input(
                    "Enter Customer Address:", "123 Mane St, Anytown, USA"
                )
                if entered_address != correct_address:
                    st.error(
                        f"Accuracy Issue: The entered address '{entered_address}' does not match the correct address '{correct_address}'."
                    )
                else:
            ...
            with st.expander("Interactive Example"):
                ...
                if not all([name, email, phone]):
                    st.error(
                        "Completeness Issue:  Missing required fields.  Name, email, and phone are mandatory."
                    )
                else:
            ...
            with st.expander("Interactive Example"):
                ...
                if system1_name != system2_name:
                    if system1_name.replace(" ", "") == system2_name.replace(
                        " ", ""
                    ).replace(
                        ",", ""
                    ):  # handle formatting
                        st.warning(
                            f"Consistency Issue: Name format is different, but values represent the same entity."
                        )
                    else:
                        st.error(
                            f"Consistency Issue: Customer name is different across systems: '{system1_name}' vs. '{system2_name}'."
                        )
                else:
                    st.success(
                        "Consistency Check: Customer name is consistent across systems."
                    )
            ...
            with st.expander("Interactive Example"):
                data_needed_by = pd.to_datetime(
                    "2024-01-15"
                )  # Example: Data needed by Jan 15, 2024
                data_available_on = pd.to_datetime(
                    st.date_input(
                        "Data Available On:", value=pd.to_datetime("2024-01-16")
                    )
                )

                if data_available_on > data_needed_by:
                    st.error(
                        f"Timeliness Issue: Data was needed by {data_needed_by.strftime('%Y-%m-%d')}, but was only available on {data_available_on.strftime('%Y-%m-%d')} ."
                    )
                else:
            ...
            with st.expander("Interactive Example"):
                customer_ids = st.text_input(
                    "Enter Customer IDs (comma-separated):", "1, 2, 3, 2, 4"
                )
        ...
        framework = st.selectbox(
            "Select a Framework:", ["GDPR", "CCPA", "HIPAA", "SOX", "PCI DSS"]
        )
    ...
    quiz_questions = [
        ...
        },
        {
            ...
            "options": [
                ...
                "Managing older, often outdated data systems and the data they contain",
        ...
        },
        {
            ...
            "options": [
                "A secure storage location for data",
        ...
        },
        {
            ...
            "options": [
                ...
                "Data entries are unique with no duplicates",
                "Data entries follow required constraints and formatting",
                "Data across databases is free of contradictions",
            ],
