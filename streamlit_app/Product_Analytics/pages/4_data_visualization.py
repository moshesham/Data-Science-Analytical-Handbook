...
def data_visualization_page():
    ...
    st.write("Master data visualization to explore, analyze, and communicate data effectively.")
    ...
    with st.expander("📖 Theoretical Concepts and Best Practices"):
        ...
        chart_types_data = {
            "Chart Type": [
                "Histogram", "Box Plot", "Density Plot", "Scatter Plot", "Line Chart",
                "Bar Chart", "Stacked Bar Chart", "Heatmap", "Violin Plot", "Pie Chart",
                "Area Chart", "Scatter Plot Matrix", "3D Scatter Plot", "Bubble Chart", "Funnel Chart"
            ],
            "Data Types": [
                "Numerical (Continuous or Discrete)", "Numerical, (Optional) Categorical",
                "Numerical (Continuous)", "Numerical (Two Variables)", "Time Series (Numerical vs. Time)",
                "Categorical, (Optional) Numerical", "Categorical, Numerical",
                "Numerical (Multiple), Categorical (2D Grid)", "Numerical, (Optional) Categorical",
                "Categorical (Proportions)", "Time Series (Numerical vs. Time)",
                "Numerical (Multiple)", "Numerical (Three Variables)",
                 "Numerical (Three Variables), (Optional) Categorical", "Numerical (Sequential Stages)"
    ...
    st.write("Select a visualization and explore its parameters. Use the example dataset or upload your own.")


    data_source = st.radio("Data Source:", ["Use Example Dataset", "Upload CSV", "Paste CSV Data"], horizontal=True)
    ...
    if not numerical_cols:
        st.warning("No numerical columns found. Many visualizations require numerical data.")

    viz_type = st.selectbox("Visualization Type", [
        "Histogram", "Box Plot", "Density Plot", "Scatter Plot", "Line Chart",
        "Bar Chart", "Heatmap", "Violin Plot", "Scatter Plot Matrix", "3D Scatter Plot", "Bubble Chart", "Funnel Chart"
    ], index=0)
    ...
    if viz_type == "Histogram":
        ...
        nbins = st.slider("Number of Bins:", min_value=5, max_value=100, value=30, step=5)
        ...
        if st.button("Plot Histogram", key="histogram_button"):
            fig = px.histogram(data, x=col, nbins=nbins, title=f"Histogram of {col}",
        ...
        col = st.selectbox("Select Column:", numerical_cols)
        group_col = st.selectbox("Group By (Optional):", ["None"] + list(data.select_dtypes(include='object').columns))

        if st.button("Plot Box Plot", key="boxplot_button"):
            fig = px.box(data, x=group_col if group_col != "None" else None, y=col,
                         title=f"Box Plot of {col}" + (f" by {group_col}" if group_col != "None" else ""),
                         points="all")
            st.plotly_chart(fig, use_container_width=True)
        ...
        if st.button("Plot Density Plot", key="density_button"):
            fig = px.histogram(data, x=col, title=f"Density Plot of {col}", marginal="rug", histnorm='probability density')
            st.plotly_chart(fig, use_container_width=True)
        ...
        if len(numerical_cols) < 2:
            ...
            y_col = st.selectbox("Select Y-axis Column:", numerical_cols, index=1 if len(numerical_cols) > 1 else 0)
            color_col = st.selectbox("Color by (Optional):", ["None"] + list(data.columns), index=0)
            ...
            if st.button("Plot Scatter Plot", key="scatter_button"):
                fig = px.scatter(data, x=x_col, y=y_col, color=color_col if color_col != "None" else None,
                                 title=f"Scatter Plot of {x_col} vs. {y_col}", trendline=trendline if trendline != "None" else None,
                                 opacity=0.7)
                st.plotly_chart(fig, use_container_width=True)

    elif viz_type == "Line Chart":
        st.subheader("Line Chart")
        date_cols = data.select_dtypes(include=['datetime', 'datetimetz']).columns.tolist()
        if not date_cols:
            ...
            agg_func = st.selectbox("Aggregation Function:", ["sum", "mean", "median", "count"], index=0)
            if st.button("Plot Line Chart", key="line_button"):
                plot_data = data.groupby(time_col)[value_col].agg(agg_func).reset_index()
                plot_data = plot_data.sort_values(by=time_col)
                fig = px.line(plot_data, x=time_col, y=value_col, title=f"Line Chart of {value_col} over Time")
                st.plotly_chart(fig, use_container_width=True)

    elif viz_type == "Bar Chart":
        st.subheader("Bar Chart")
        cat_cols = st.multiselect("Select Category Column(s):", data.select_dtypes(include='object').columns)
        num_col = st.selectbox("Select Value Column (Optional - for aggregation):", ["None"] + list(data.select_dtypes(include=np.number).columns))
        ...
        if st.button("Plot Bar Chart", key="bar_button"):  # UNIQUE KEY
            ...
            if num_col != "None":
                agg_func = st.selectbox("Aggregation Function:", ["sum", "mean", "count", "median", "min", "max"], index=0)
                agg_functions = {
                    'sum': 'sum',
                    'mean': 'mean',
                    'count': 'count',
                ...
                agg_data = data.groupby(cat_cols)[num_col].agg(agg_functions[agg_func]).reset_index()

                if len(cat_cols) == 1:
                    fig = px.bar(agg_data, x=cat_cols[0], y=num_col,
                                 title=f"{agg_func.capitalize()} of {num_col} by {cat_cols[0]}",
                                 orientation='v' if orientation == 'Vertical' else 'h',
                                 color=cat_cols[0] if len(agg_data[cat_cols[0]].unique()) <= 10 else None)
                else:
                    agg_data['combined_categories'] = agg_data[cat_cols].astype(str).agg(' - '.join, axis=1)
                    fig = px.bar(agg_data, x='combined_categories', y=num_col,
                                 title=f"{agg_func.capitalize()} of {num_col} by {', '.join(cat_cols)}",
                                 orientation='v' if orientation == 'Vertical' else 'h',
                                 color='combined_categories' if len(agg_data['combined_categories'].unique()) <= 10 else None)
                ...
                if len(cat_cols) == 1:
                    ...
                    fig = px.bar(counts, x=cat_cols[0], y='Count',
                                 title=f"Counts of {cat_cols[0]}",
                                 orientation='v' if orientation == 'Vertical' else 'h',
                                 color=cat_cols[0] if len(counts[cat_cols[0]].unique()) <= 10 else None)
                else:
                    counts = data.groupby(cat_cols).size().reset_index(name='Count')
                    counts['combined_categories'] = counts[cat_cols].astype(str).agg(' - '.join, axis=1)
                    fig = px.bar(counts, x='combined_categories', y='Count',
                                  title=f"Count by {', '.join(cat_cols)}",
                                  orientation='v' if orientation == 'Vertical' else 'h',
                                  color='combined_categories' if len(counts['combined_categories'].unique()) <= 10 else None)
                    fig.update_xaxes(tickangle=-45)
                st.plotly_chart(fig, use_container_width=True)
        ...
        x_col = st.selectbox("Select X-axis (Categorical):", ["None"] + list(data.select_dtypes(include='object').columns))
        y_col = st.selectbox("Select Y-axis (Numerical):", numerical_cols)

        if st.button("Plot Violin Plot", key="violin_button"): # Unique Key
            fig = px.violin(data, x=x_col if x_col != "None" else None, y=y_col,
        ...
        if st.button("Generate Scatter Plot Matrix", key="scatter_matrix_button"): # Unique Key
            if len(cols) < 2:
                st.error("Please select at least two numerical columns.")
            else:
                fig = px.scatter_matrix(data, dimensions=cols, title="Scatter Plot Matrix")
                st.plotly_chart(fig, use_container_width=True)
        ...
        if len(numerical_cols) < 3:
            ...
            x_col = st.selectbox("Select X-axis Column:", numerical_cols, index=0)
            y_col = st.selectbox("Select Y-axis Column:", numerical_cols, index=1 if len(numerical_cols) > 1 else 0)
            z_col = st.selectbox("Select Z-axis Column:", numerical_cols, index=2 if len(numerical_cols) > 2 else 0)
            color_col = st.selectbox("Color by (Optional):", ["None"] + list(data.columns), index=0)
        ...
        if len(numerical_cols) < 3:
            st.error("At least three numerical columns are required for a Bubble Chart.")
        else:
            x_col = st.selectbox("Select X-axis Column:", numerical_cols, index=0)
            y_col = st.selectbox("Select Y-axis Column:", numerical_cols, index=1 if len(numerical_cols) > 1 else 0)
            size_col = st.selectbox("Select Size Column:", numerical_cols, index=2 if len(numerical_cols) > 2 else 0)
            color_col = st.selectbox("Color by (Optional):", ["None"] + list(data.columns), index=0)

            if st.button("Generate Bubble Chart", key="bubble_chart_button"): # Unique Key
        ...
        num_stages = st.number_input("Number of Stages:", min_value=2, max_value=10, value=4, step=1)
        stages = []
        values = []
        for i in range(num_stages):
            stage = st.text_input(f"Stage {i+1} Name:", value=f"Stage {i+1}")
            value = st.number_input(f"Value for {stage}:", min_value=0, value=1000 - (i * 100), step=10)
            stages.append(stage)
            values.append(value)
    ...
    st.write("This dashboard demonstrates how visualizations combine to provide a comprehensive view.  It's a simplified example.")
    ...
    if st.button("Generate Dashboard", key="dashboard_button"):
        ...
        with col3:
            try:
                conversion_rate = data['DeviceType'].value_counts(normalize=True)['Mobile'] * 100  # Example
        ...
        try:
            daily_data = data.groupby('Date')['DailyClicks'].sum().reset_index()  # Example
            fig_time = px.line(daily_data, x='Date', y='DailyClicks', title="Daily Active Users (Proxy)")
        ...
        try:
            fig_hist = px.histogram(data, x='SessionDuration', title="Distribution of Session Durations", nbins=30, marginal="box")
        ...
        try:
            ...
            fig_bar = px.bar(user_counts, x='Category', y='Count', title="Users by Category")
        ...
        if len(numerical_cols) >= 2:
            corr_matrix = data[numerical_cols].corr()
            fig_heatmap = px.imshow(corr_matrix, text_auto=".2f", aspect="auto", color_continuous_scale="RdBu_r", title="Correlation Heatmap")
