...
def create_dirty_data(
    ...
    for col in df.columns:
        mask = np.random.choice(
            [True, False], n_rows, p=[missing_rate, 1 - missing_rate]
        )
    ...
    for col in ["Age", "Income"]:
        ...
        df.loc[outlier_indices, col] *= np.random.choice(
            [5, -5], num_outliers
        )  # make them very high

    # Introduce inconsistencies (in categorical and date columns)
    df["Purchase"] = df["Purchase"].str.lower()
    inconsistency_indices = np.random.choice(
        df.index, int(n_rows * inconsistency_rate), replace=False
    )
    df.loc[inconsistency_indices, "Purchase"] = df.loc[
        inconsistency_indices, "Purchase"
    ].str.upper()

    date_inconsistency_indices = np.random.choice(
        df.index, int(n_rows * inconsistency_rate), replace=False
    )
    df.loc[date_inconsistency_indices, "Date"] = df.loc[
        date_inconsistency_indices, "Date"
    ].apply(lambda x: x + pd.DateOffset(years=10) if pd.notna(x) else x)
...
def main():
    st.set_page_config(
        page_title="Data Validation and Cleaning", page_icon="🧼", layout="wide"
    )
    ...
    data_source = st.radio(
        "Choose data source:", ["Generate Synthetic Data", "Upload CSV"]
    )
    if data_source == "Generate Synthetic Data":
        n_rows = st.slider(
            "Number of Rows:", min_value=50, max_value=500, value=100, step=50
        )
        missing_rate = st.slider(
            "Missing Value Rate:", min_value=0.0, max_value=0.5, value=0.1, step=0.05
        )
        outlier_rate = st.slider(
            "Outlier Rate:", min_value=0.0, max_value=0.2, value=0.05, step=0.05
        )
        inconsistency_rate = st.slider(
            "Inconsistency Rate:", min_value=0.0, max_value=0.2, value=0.05, step=0.05
        )
    ...
    if df is not None:  # Only proceed if we have a valid df
        ...
        with st.expander("Outliers"):
            ...
            for col in df.select_dtypes(include=["number"]):
                ...
                outlier_handling = st.selectbox(
                    f"Handle Outliers in {col}:",
                    ["None", "Remove", "Winsorize (95th percentile)"],
                    key=f"outlier_{col}",
                )
        ...
        with st.expander("Inconsistencies"):
            ...
            for col in df.select_dtypes(include="object"):
                ...
                inconsistency_handling = st.selectbox(
                    f"Handle Inconsistencies in {col}:",
                    ["None", "Convert to Lowercase", "Convert to Uppercase"],
                    key=f"inconsistency_{col}",
                )
                ...
                if inconsistency_handling != "None":  # display after handling
                    st.write(
                        f"Unique values in {col} after handling:", df[col].unique()
                    )
        ...
        with st.expander("Incorrect Data Types"):
            ...
            for col in df.columns:
                ...
                new_type = st.selectbox(
                    f"Change data type of '{col}':", available_types, key=f"dtype_{col}"
                )
                if (
                    new_type != "infer" and str(current_type) != new_type
                ):  # only change if different type
                    try:
            ...
            for col in df.columns:
                if df[col].dtype in ["int64", "float64"]:
                    min_val = st.number_input(
                        f"Minimum valid value for '{col}':",
                        value=df[col].min(),
                        key=f"min_{col}",
                    )  # Defaults shown
                    max_val = st.number_input(
                        f"Maximum valid value for '{col}':",
                        value=df[col].max(),
                        key=f"max_{col}",
                    )
                    if st.button(
                        f"Filter Invalid Values in {col}", key=f"filter_{col}"
                    ):
    ...
    quiz_questions = [
        ...
        },
        {
            ...
            "options": [
                ...
                "Convert it to an appropriate numerical or datetime type",
                "Remove the column",
            ],
        ...
        },
        {
            ...
            "options": [
                ...
                "Converting the variable to a categorical type",
            ],
        ...
        },
        {
            ...
            "options": [
                "Immediately remove all rows with missing values.",
                "Impute all missing values with the mean.",
                "Always remove outliers.",
                "Understand the *reason* for missing values and outliers.",
            ],
