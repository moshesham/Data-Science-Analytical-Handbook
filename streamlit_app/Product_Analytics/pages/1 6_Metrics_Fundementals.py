...
def main():
    st.set_page_config(page_title="Creating Effective KRIs and KPIs", page_icon="🎯", layout="wide")

    st.title("Creating Informative KRIs and KPIs")
    st.write("Learn how to define, design, and implement effective Key Risk Indicators (KRIs) and Key Performance Indicators (KPIs).")
    ...
    if indicator_type == "KPI":
        ...
        # Step 1: Objective
        objective = st.text_input("1. Strategic Objective:", "Increase customer satisfaction")

        # Step 2: KPI Definition
        kpi_name = st.text_input("2. KPI Name:", "Customer Satisfaction Score")
        kpi_definition = st.text_area("3. KPI Definition:", "The average customer satisfaction rating based on a survey (1-5 scale).")
        kpi_formula = st.text_input("4. KPI Formula:", "Total Satisfaction Ratings / Number of Respondents")

        # Step 3: Target
        target_type = st.selectbox("Target Type:", ["Specific Value", "Range", "Improvement over Baseline"])
        if target_type == "Specific Value":
            ...
            upper_bound = st.number_input("Upper Bound:", min_value=lower_bound, value=4.8) #Ensure upper>lower
        elif target_type == "Improvement over Baseline":
            baseline_value = st.number_input("Baseline Value:", min_value=0.0, value=3.8)
            improvement_percentage = st.number_input("Improvement Percentage:", min_value=0.0, value=10.0)
        ...
        data_source = st.text_input("5. Data Source:", "Customer satisfaction survey responses")
        collection_frequency = st.selectbox("6. Collection Frequency:", ["Daily", "Weekly", "Monthly", "Quarterly", "Annually"])
        data_owner = st.text_input("7. Data Owner:", "Customer Service Department")

        # Step 5: Reporting and Monitoring
        reporting_frequency = st.selectbox("8. Reporting Frequency:", ["Daily", "Weekly", "Monthly", "Quarterly", "Annually"])
        reporting_format = st.text_input("9. Reporting Format:", "Dashboard, email report")
        review_frequency = st.selectbox("10. Review Frequency:", ["Monthly", "Quarterly", "Annually"])
        ...
        if st.button("Generate KPI Summary"):
            ...
            if target_type == "Specific Value":
              ...
              st.markdown(f"**Target:** {target_value:.2f}") #display calculated target
            ...
            smart_assessment = {
                ...
                "Time-bound": "✅" if collection_frequency and reporting_frequency else "❌",
            }
        ...
        kri_definition = st.text_area("3. KRI Definition:", "The number of failed login attempts per user per day.")
        kri_formula = st.text_input("4. KRI Formula:", "Total Failed Login Attempts / Number of Users")

        # Step 3: Thresholds
        threshold_type = st.selectbox("Threshold Type", ["Upper Limit", "Lower Limit", "Range"])
        ...
        if threshold_type == "Upper Limit":
            ...
            upper_threshold = st.number_input("Upper Threshold:", min_value=lower_threshold, value=5) #ensure upper > lower

        # Step 4: Data Source and Collection
        data_source = st.text_input("5. Data Source:", "System login logs")
        collection_frequency = st.selectbox("6. Collection Frequency:", ["Real-time", "Hourly", "Daily", "Weekly"])
        data_owner = st.text_input("7. Data Owner:", "IT Security Department")

        # Step 5: Reporting and Monitoring
        reporting_frequency = st.selectbox("8. Reporting Frequency:", ["Real-time", "Hourly", "Daily", "Weekly"])
        reporting_format = st.text_input("9. Reporting Format:", "Dashboard, alert notification")
        review_frequency = st.selectbox("10. Review Frequency:", ["Monthly", "Quarterly", "Annually"])
        ...
        if st.button("Generate KRI Summary"):
            ...
            if threshold_type == "Upper Limit":
              ...
              st.markdown(f"**Threshold Range:** {lower_threshold} - {upper_threshold}")
    ...
    * **Healthcare:** Patient Wait Times (KPI), Hospital-Acquired Infection Rate (KRI)
    ...
    quiz_questions = [
    {
        ...
        "options": ["KPIs measure performance, while KRIs measure risk.",
    ...
    },
    {
        ...
        "options": ["Number of phone calls answered", "Average call handling time", "Customer satisfaction score", "Number of customer complaints"],
    ...
    },
    {
        ...
        "options": ["Number of employees trained on security awareness",
    ...
    },
    {
        ...
        "options": ["Setting targets that are too easy to achieve",
    ...
    },
     {
        ...
        "options":["Having 100's of KPIs and KRIs so everything is monitored", "Having a small number of key indicators"],
    ...
    },
    {
        ...
        "options":["An indicator that measures past performance", "An indicator that predicts future performance or risk"],
    ...
    },
    {
        ...
        "options":["A value that when crossed signals a significant risk", "A target or acceptable range for a KPI"],
    ...
    },
     {
        ...
        "options":["A value that when crossed signals a significant risk", "A target or acceptable range for a KRI"],
