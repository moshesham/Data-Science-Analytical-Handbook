...
def main():
    ...
    st.write(
        "Learn the fundamentals of hypothesis testing, including null and alternative hypotheses, p-values, and statistical significance."
    )
    ...
    st.write(
        "Explore hypothesis testing with an interactive simulation. Experiment with the different parameters to see how they affect the outcome of each test."
    )
    ...
    if test_type == "One Sample T-test":
        ...
        pop_mean = st.number_input(
            "Population Mean", value=10.0, help="The known population mean."
        )
        ...
        effect_size = st.slider(
            "Effect Size (difference from population mean)",
            min_value=-2.0,
            max_value=2.0,
            value=0.5,
            step=0.1,
            help="The true difference from the population mean.",
        )
        alpha = st.slider(
            ...
            value=0.05,
            step=0.01,
            help="The significance level used for hypothesis testing.",
        )
        power = st.slider(
            "Desired Statistical Power (1-Type 2 Error)",
            min_value=0.5,
            max_value=0.99,
            value=0.8,
            step=0.01,
            help="Probability of rejecting the null hypothesis when it is false.",
        )

        if st.button(
            "Run One-Sample T-test Simulation",
            help="Simulate a T-test with provided inputs",
        ):
            with st.spinner("Running the simulation..."):
                ...
                sample = np.random.normal(
                    loc=sample_mean_true, scale=2, size=sample_size
                )
                ...
                if p_value < alpha:
                    st.success(
                        "Reject null hypothesis. The sample mean is significantly different from the population mean."
                    )
                else:
                    st.error(
                        "Fail to reject null hypothesis. There is not a significant difference."
                    )
                sample_size_power = stats_utils.calculate_sample_size_power(
                    effect_size=effect_size, power=power, alpha=alpha
                )
                st.write(
                    f"**Sample Size Needed (for given power):** {sample_size_power}"
                )

                # Example histogram to visualize
                fig = viz_utils.plot_histogram(
                    sample, title="Sample Data", xlabel="Values", ylabel="Frequency"
                )
                plt.axvline(pop_mean, color="r", linestyle="dashed", label="Pop. mean")
                plt.axvline(
                    np.mean(sample), color="g", linestyle="dashed", label="Sample mean"
                )
                plt.legend()

                st.pyplot(fig)
                st.write(
                    "The histogram above shows the simulated data and the population mean as a dashed line."
                )
    elif test_type == "Two Sample T-test":
        st.subheader("Two Sample T-test Simulation")
        group_a_size = st.slider(
            "Sample Size Group A",
            min_value=30,
            max_value=500,
            value=100,
            step=10,
            help="The sample size for group A.",
        )
        group_b_size = st.slider(
            "Sample Size Group B",
            min_value=30,
            max_value=500,
            value=100,
            step=10,
            help="The sample size for group B.",
        )
        mean_diff = st.slider(
            ...
            value=0.5,
            step=0.1,
            help="The true difference in mean between group A and B.",
        )
        alpha = st.slider(
            "Significance Level (alpha)",
            min_value=0.01,
        ...
        if st.button(
            "Run Two Sample T-test Simulation",
            help="Simulate a two sample t-test with provided values.",
        ):
            with st.spinner("Running the simulation..."):
                group_a = np.random.normal(loc=10, scale=2, size=group_a_size)
                group_b = np.random.normal(
                    loc=10 + mean_diff, scale=2, size=group_b_size
                )
                ...
                sample_size_power = stats_utils.calculate_sample_size_power(
                    effect_size=mean_diff, power=power, alpha=alpha
                )
                st.write(
                    f"**Sample Size Needed (for given power, for each group):** {sample_size_power:.2f}"
                )

                # Visualize
                data_sets = {"Group A": group_a, "Group B": group_b}
                fig = viz_utils.plot_multiple_histograms(
                    data_sets, title="Group A vs B Histogram"
                )
                st.pyplot(fig)
                st.write(
                    "The histograms above show the distribution of the two simulated groups."
                )

    elif test_type == "Chi-square Test":
        st.subheader("Chi-Square Test Simulation")
        st.write("Enter a table of observed counts below.")
        observed_counts_str = st.text_area(
            "Observed Counts (Comma-separated rows, rows separated by new lines)",
            value="30,70\n40,60",
            help="Enter a table of observed counts.",
        )
        alpha = st.slider(
            "Significance Level (alpha)",
            min_value=0.01,
        ...
        if st.button(
            "Run Chi-Square Test",
            help="Perform the chi-square test with the observed counts provided.",
        ):
            try:
                observed_counts = np.array(
                    [
                        [int(x.strip()) for x in row.split(",")]
                        for row in observed_counts_str.splitlines()
                    ]
                )
                # Perform the test
                ...
                if p_value_chi < alpha:
                    st.success(
                        "Reject the null. There is an association between the groups and the event."
                    )
                else:
                    st.error(
    ...
    st.write(
        "Test your understanding of hypothesis testing with the following exercises."
    )

    st.subheader("Exercise 1: One Sample T-Test")
    st.write(
        "A company claims their product has a mean rating of 4.5. You collect data from 100 users and find a sample mean of 4.3 with a standard deviation of 0.8. Perform a one sample t-test to see if this claim is true. (Assume a significance level of 0.05)."
    )
    if st.button("Check Solution for Exercise 1"):
        ...
        t_stat_exercise = (sample_mean - pop_mean_exercise) / (
            sample_std_dev / np.sqrt(sample_size_exercise)
        )
        p_value_exercise = 2 * (
            1 - stats.t.cdf(np.abs(t_stat_exercise), df=sample_size_exercise - 1)
        )
    ...
    if st.button("Check Solution for Exercise 2"):
        ...
        t_stat_exercise, p_value_exercise = stats.ttest_ind(
            np.random.normal(mean_a, std_a, sample_size_a),
            np.random.normal(mean_b, std_b, sample_size_b),
        )
        st.write(f"**T Statistic:** {t_stat_exercise:.3f}")
        st.write(f"**P Value:** {p_value_exercise:.3f}")
        if p_value_exercise < alpha_exercise:
