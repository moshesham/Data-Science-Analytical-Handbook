...
**→ [Post Your Daily Check-In](https://github.com/moshesham/Data-Science-Analytical-Handbook/discussions/categories/2026-challenge)**
...
| 8 | [Capstone Presentations 🎓](https://github.com/moshesham/Data-Science-Analytical-Handbook/discussions/new?category=2026-challenge&title=Week%208%20Capstone) |
...
**Key Insight:** The `ROWS BETWEEN 6 PRECEDING AND CURRENT ROW` creates a 7-day window (current + 6 prior). The `days_in_window` column shows how many days are actually in the calculation—useful for the first 6 days when the window is partial.
...
**Scenario:** Your PM asks: "We ran an A/B test and got a p-value of 0.03. The data scientist says it's significant but we should wait for more data. That doesn't make sense—can you explain?"
...
The data scientist likely wants to wait until we hit our pre-planned sample size to avoid this 'peeking' problem. Think of it like not opening the oven door while the cake is baking—checking early can ruin the result."
...
**Problem:** You're planning an A/B test for a checkout flow change. Current conversion rate is 3.2%. You want to detect a 10% relative lift (to 3.52%) with 80% power at α=0.05.
...
- Control conversion: 4.2% (95% CI: 3.9% – 4.5%)
- Treatment conversion: 4.6% (95% CI: 4.3% – 4.9%)

The PM says "Treatment is clearly better—the CIs don't overlap!"
...
| Scenario | Effect | Why |
|----------|--------|-----|
| A. 80% → 90% power | **INCREASES** | Need more data to reduce false negatives |
| B. α from 0.05 → 0.01 | **INCREASES** | Stricter threshold requires more evidence |
| C. Baseline 5% → 50% | **INCREASES** | Higher variance at p=0.5 (p(1-p) maximized) |
| D. MDE 5% → 10% | **DECREASES** | Larger effects are easier to detect |
| E. Two-tailed → one-tailed | **DECREASES** | All alpha is on one side; easier to detect |
...
<details>
<summary>✅ Sample Design Document</summary>
...
| Guardrail | Concern |
|-----------|---------|
| DAU | Feature feels spammy → users leave |
| Time spent per session | Users spend time on Super Like UI → less feed time |
...
- MDE: 3% relative → 0.024 absolute increase
- Variance: Assume Poisson, σ² ≈ μ → σ = 0.89
- With α=0.05, power=80%: ~50,000 users per variant

**6. Duration:**
- Minimum 2 weeks (full weekly cycle × 2)
- Extend if:
...
|--------|---------|-----------|
| Conversion ✅ | Good | +7.8% is statistically significant and meaningful |
| Revenue ⚠️ | Neutral | No significant change; concerning given conversion increase |
| Page Load ❌ | Bad | 50% slower is a major UX regression and SEO risk |
...
**Result:** p < 0.001 → **This IS a significant SRM.** The split is not random.
...
| Browser/device exclusion | Treatment feature doesn't work on Safari → those users excluded |
| Redirect-based assignment | Slower treatment page → more users bounce before logging |
| Experiment start time | Gradual rollout started at different times |

**3. What to Do:**

1. **STOP the experiment** — results are untrustworthy
...
- Primary metric (purchases): +5% (p=0.02) ✅
- Guardrail (customer service tickets): +15% (p=0.01) ❌
...
**Example Analysis:**
```
Incremental purchases: 5% × 1M purchases × $50 avg = $2.5M
Incremental tickets: 15% × 50K tickets × $20 cost each = $150K
Net gain: $2.35M
```
...
*"I recommend we investigate before shipping. The +15% in support tickets isn't just a cost issue—it's a signal that something in the feature is confusing or broken. If we ship without understanding why, we risk:*
...
print(f"Merged: {len(merged)}")  # Should equal len(orders) for inner, ≥ len(users) for left
...
<details>
<summary>✅ Better Approaches</summary>
...
| 38 | Case Study Walk-Through | Crack one open-ended case: "DAU dropped 5%—investigate" | [Handbook: Analytical Execution]({{ '/interview-preparation/analytical-execution/' | relative_url }}) |
...
<details>
<summary><strong>Challenge 1: DAU Dropped 5% — Full Investigation Framework</strong></summary>
...
<details>
<summary>✅ Complete Framework</summary>
...
Or: DAU = Σ (Users by Platform) = iOS + Android + Web

Or: DAU = Σ (Users by Country) = US + UK + India + ...

Or: DAU = Σ (Users by Acquisition Source) = Organic + Paid + Referral
```
...
**Scenario:** Instagram is launching "Close Friends Story Reactions" — when you react to a close friend's story, they see a special animation. You're asked to define success metrics.

**Your Task:** Define a complete metrics framework.

<details>
<summary>✅ Complete Metrics Framework</summary>

### North Star Metric

**Close Friends Interactions per User per Week**

Why: This measures the core behavior we're trying to drive — deeper connections with close friends.
...
<details>
<summary>✅ Structured Recommendation</summary>
...
- **Customer service tickets up 50%** — this is a major red flag
- Page load time increased 10% — impacts SEO and user experience
...
1. **Investigate support ticket content** — What are users complaining about?
2. **Analyze conversion quality** — Are these incremental converters or just shifted from returning later?
3. **Fix page load regression** — This is likely addressable
4. **Consider partial rollout** — Launch to power users first, gather qualitative feedback
...
<details>
<summary><strong>Challenge 4: "Metric Went Up — Is It Good?" (Nuance Exercise)</strong></summary>
...
| DAU/MAU ratio | +10% | **Good** | Higher stickiness — users engage more frequently. Classic engagement metric. |
...
| Day | Focus | Do This | Resource |
|-----|-------|---------|----------|
| 43 | Raw → Staging → Mart Layers | Read the AE index page; sketch layers for an orders pipeline | [Handbook: Analytical Engineering]({{ '/analytical-engineering/' | relative_url }}) |
...
- `raw_orders` — every order placed (may have duplicates from retry logic)
- `raw_order_items` — line items within orders
...
<details>
<summary>✅ Complete Pipeline Design</summary>
...
├── int_order_items_enriched  (joined with product info)
    ↓
...
**Step 1: Staging — Deduplicate Orders**
```sql
...
**Step 2: Staging — Clean Order Items**
```sql
...
**Step 4: Intermediate — Enrich Order Items**
```sql
...
-- Test 3: Referential integrity — all products exist
-- Expected: 0 rows
...
-- Bonus: Aggregate test — revenue should match
...
### 📊 Part 2: Hypothesis Testing (Python)
...
### 📈 Part 3: Feature Importance (Python)
...
### 🎤 Part 4: The Presentation (5 Slides)
...
- Analysis: SQL exploration → Statistical testing → Feature modeling
- Validation: 80/20 train-test split

#### Slide 3: Key Insight
> "**Bedrooms and capacity drive 60% of pricing** — but Superhost status only adds a $12 premium despite the badge."
...
├── README.md                 # Overview, key findings, how to run
├── data/
│   └── listings.csv          # Raw data (or download script)
├── notebooks/
│   ├── 01_exploration.ipynb  # SQL-like exploration in Python
│   ├── 02_hypothesis_tests.ipynb
│   └── 03_feature_analysis.ipynb
├── sql/
│   └── exploration_queries.sql
├── presentation/
│   └── airbnb_pricing.pdf    # Final slides
└── requirements.txt
```
...
- ❌ "Users in California have higher LTV than Texas" — So what?
- ✅ "Users in California have 40% higher LTV, which means we should increase CAC budget for CA by 20%" — Clear action!
...
<div class="cta-box" style="background: linear-gradient(135deg, #11998e 0%, #38ef7d 100%); padding: 2rem; border-radius: 12px; color: white; text-align: center; margin: 2rem 0;">
  <h2 style="color: white; margin-top: 0;">🎯 Start Your Journey Today</h2>
  <p style="font-size: 1.1rem;">Don't wait for January 1st—start preparing now!</p>
